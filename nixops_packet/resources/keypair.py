    ...
    def create(self, defn, check, allow_reboot, allow_recreate):
        ...
        if check or self.state != self.UP:
            ...
            if not kp or self.state != self.UP:
                ...
                kp = self._connection().create_project_ssh_key(self.project, defn.keypair_name, self.public_key)
    ...
    def destroy(self, wipe=False):
        def keypair_used():
            for m in self.depl.active_resources.values():
                if isinstance(m, nixops_packet.backends.device.PacketState) and m.key_pair == self.keypair_name:
                    return m
            return None
